...
<head>
<meta charset="UTF-8">
<title>Сводный анализ пользователей Habr</title>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
  <header class="py-8">
    <h1 class="text-4xl font-bold mb-2">Сводный анализ по всем пользователям</h1>
...
<head>
<meta charset="UTF-8">
<title>Анализ пользователей Habr</title>
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<script src="https://cdn.tailwindcss.com?plugins=forms,typography"></script>
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
...
<body class="bg-gray-50">
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
  <header class="py-8"><h1 class="text-4xl font-bold mb-2">Анализ активности пользователей Habr</h1><p class="text-lg text-gray-500">Сгенерировано: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p></header>
  <nav class="sticky top-0 z-10 bg-gray-50/80 backdrop-blur-sm py-3 mb-8 border-y border-gray-200"><div class="flex items-center flex-wrap"><span class="font-bold mr-4">Пользователи:</span>{nav_links_html}</div></nav>
...
def main():
    user_dir = 'user'
    json_files = glob.glob(os.path.join(user_dir, '**/*.json'), recursive=True)

    if not json_files:
        print(f"Директория '{user_dir}' не найдена или пуста. Пожалуйста, создайте ее и поместите в нее JSON-файлы пользователей.")
        return
    ...
    for file_path in json_files:
        ...
        profiles_data.append({
            'name': profile_name, 'total_articles': int(total_articles), 'avg_monthly_20_25': round(float(avg_monthly), 2),
    ...
    if profiles_data:
        ...
        avg_median_views_per_year = {year: sum(c['median_views_per_year_20_25'][year] for c in profiles_data) / num_profiles for year in years}
        avg_median_reactions_per_year = {year: sum(c['median_reactions_per_year_20_25'][year] for c in profiles_data) / num_profiles for year in years}
        ...
        with open('aggregated_users_analysis.html', 'w', encoding='utf-8') as f:
            f.write(agg_html_content)
        print("Сводный анализ сохранен в файле: aggregated_users_analysis.html")

    html_content = render_analysis_html(profiles_data, aggregated_data)
    output_filename = 'users_analysis.html'
